...
exp_data = np.loadtxt('BMMnnet_input_t6_2013-01-03-06-30:00.txt',usecols=[5])


pp = PdfPages("BMMnnet_wp_in-sample_pred_t6_2013-01-03-06-30:00.pdf")
plt.figure (0)
plt.clf()
plt.plot(exp_data[0:nn],label="EXP")

exp_in_obs = np.loadtxt('BMMnnet_input_t6_2013-01-03-06-30:00.txt',usecols=[5])
...
plt.ylim(min(np.concatenate((exp_in_obs,exp_in_average_low), axis=1))-50, max(np.concatenate((exp_in_obs,exp_in_average_high), axis=1))+50)

plt.xlim(1,nn)

plt.legend(loc='upper right')

plt.ylabel('Wind power', fontsize=14)
...
pp = PdfPages("BMMnnet_wp_out-sample_pred_t6_2013-01-03-06-30:00.pdf")
plt.figure (1)
plt.clf()
...
exp_out_obs = np.loadtxt('BMMnnet_output_out-sample_t6_2013-01-03-06-30:00.csv',usecols=[5])
...
plt.ylim(min(np.concatenate((exp_out_obs,exp_in_average_low), axis=1))-50, max(np.concatenate((exp_out_obs,exp_in_average_high), axis=1))+50)
...
pp = PdfPages("BMMnnet_wp_in-sample_wgt_pred_t6_2013-01-03-06-30:00.pdf")
plt.figure (2)
plt.clf()
#plt.plot(exp_data[0:nn],label="EXP")
...
pp = PdfPages("BMMnnet_wp_out-sample_wgt_pred_t6_2013-01-03-06-30:00.pdf")
plt.figure (4)
plt.clf()
